...
Original file is located at
    https://colab.research.google.com/drive/19hMpRL_lt8xaEdtfPgMt1D7HXJfUlXBd
"""
...
HEXAGRAM_DATA = {
    ...
    "111100": {"id": 25, "char": "䷘", "name_cn": "无妄", "pinyin": "Wú Wàng"},
...
def perform_divination():
    ...
    if primary_hex_texts and 'gua_ci' in primary_hex_texts and primary_hex_texts['gua_ci']:
        print(f"【卦辞】：{primary_hex_texts['gua_ci']}")
    else:
        print("【卦辞】：（无可用卦辞）")

    # 显示本卦的彖传和大象传
    if primary_hex_texts and 'tuan_ci' in primary_hex_texts and primary_hex_texts['tuan_ci']:
        # 彖传可能有多行，用换行符连接后打印
        tuan_display = primary_hex_texts['tuan_ci'].replace("\n", "\n           ") # 对齐后续行
        print(f"【彖曰】：{tuan_display}")
    ...
    for i in range(5, -1, -1): # 从上爻（第6爻，索引5）往下显示
        ...
        # 显示爻辞
        if primary_hex_texts and 'yao_ci' in primary_hex_texts and \
           len(primary_hex_texts['yao_ci']) > i and primary_hex_texts['yao_ci'][i]:
    ...
    if not changing_lines_indices:
        ...
        if secondary_hex_texts and 'gua_ci' in secondary_hex_texts and secondary_hex_texts['gua_ci']:
            print(f"【卦辞】：{secondary_hex_texts['gua_ci']}")
        else:
            print("【卦辞】：（无可用卦辞）")

        # 显示变卦的彖传和大象传
        if secondary_hex_texts and 'tuan_ci' in secondary_hex_texts and secondary_hex_texts['tuan_ci']:
            tuan_display_secondary = secondary_hex_texts['tuan_ci'].replace("\n", "\n           ")
            print(f"【彖曰】：{tuan_display_secondary}")
        else:
            print("【彖曰】：（无可用彖传）")
        ...
        for i in range(5, -1, -1):
            ...
            if lines_details[i]["secondary_type"] == 1:
                ...
                stable_symbol = "--- ---"
            print(f"爻 {line_num_display}: {stable_symbol}")
            # 不再显示变卦的爻辞和小象传
